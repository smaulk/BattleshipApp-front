...
export async function apiFinishGame(gameId: number, type: GameType): Promise<AxiosNoContent> {
  return handleApiResponse<AxiosNoContent>(request.put(`/api/v1/games/${gameId}`, {type: type}));
}